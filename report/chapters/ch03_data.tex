...
\subsection{Series Length Distribution}
...
\section{Target Variable: \texttt{y\_target}}
...
\section{Feature Space: \texttt{x0}--\texttt{x85}}
...
\subsection{Adaptive Splitting for Short Series}
...
The exploratory data analysis (EDA), implemented in \texttt{01\_eda.ipynb}, systematically investigates the dataset before any modeling. This chapter presents the key findings organized thematically.

\section{Panel Structure Analysis}

\subsection{Entity and Series Counts}

The panel contains 23 unique entity codes, each with multiple sub-codes and sub-categories. Combined with 4 forecast horizons, this yields 36,923 unique time series. The distribution of series across entities is uneven---some codes contribute hundreds of series while others contribute fewer.

\subsection{Temporal Coverage}

The \texttt{ts\_index} values range from 1 to approximately 3,500+, but individual series may span only a fraction of this range. The heterogeneous start times and lengths create a \emph{ragged panel}---not all series are observed at all time points.

Of the 36,923 total series, only \textbf{1,930} cross the validation cutoff at $\texttt{ts\_index} = 2880$, making them eligible for our train/validation evaluation. The remaining series either end before the cutoff or begin after it.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figures/slide_15.png}
\caption{Representative series raw plots. Two contrasting series from the panel: the longest-history series shows meaningful lag structure with visible spikes, while the highest-weight series operates at a tiny scale ($\sim 10^{-4}$) and is much harder to interpret visually. The dashed vertical line marks the validation cutoff at $\texttt{ts\_index} = 2880$.}
\label{fig:representative_series}
\end{figure}

\section{Missing Value Analysis}

The dataset contains missing values in the predictor features (\texttt{x0}--\texttt{x85}). The missingness pattern is:
...
\section{Distributional Analysis of the Target}
...
\subsection{Per-Horizon Distribution}
...
\subsection{ADF Test on First Differences}
...
\section{Autocorrelation Structure}
...
\subsection{Implications for Model Order Selection}
...
\section{Weight Concentration Analysis}
...
\subsection{Period Detection via ACF}
...
\subsection{Decomposition Findings}
...
\section{EDA Summary and Modeling Implications}
