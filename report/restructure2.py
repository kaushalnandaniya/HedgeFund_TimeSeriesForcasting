...
ch1_merged = read_file("ch01_introduction.tex")
ch3_merged = read_file("ch03_data.tex")
ch5 = read_file("ch05_classical.tex")
ch6_merged = read_file("ch06_deep.tex")
ch8 = read_file("ch08_conclusion.tex")

# Chapter 1: Introduction
new_ch1 = "\\chapter{Introduction}\n\\label{ch:introduction}\n\n"
new_ch1 += "\\section{Background and Motivation}\n"
new_ch1 += re.search(r"\\section\{Motivation and Background\}(.*?)\\section\{Project Objectives\}", ch1_merged, re.DOTALL).group(1).strip() + "\n\n"
new_ch1 += "\\section{Problem Statement}\n"
new_ch1 += re.search(r"\\section\{Competition Description\}(.*?)\\section\{Competition Timeline\}", ch1_merged, re.DOTALL).group(1).strip() + "\n\n"
new_ch1 += "\\section{Objectives}\n"
new_ch1 += re.search(r"\\section\{Project Objectives\}(.*?)\\section\{Report Structure\}", ch1_merged, re.DOTALL).group(1).strip() + "\n\n"
...
new_ch2 += re.search(r"\\section\{Data Schema\}(.*?)(\\chapter\{|$)", ch1_merged, re.DOTALL).group(1).strip() + "\n\n"
new_ch2 += "\\section{Data Characteristics}\n"
new_ch2 += re.search(r"\\section\{Dataset Overview\}(.*?)\\section\{Weight Distribution\}", ch3_merged, re.DOTALL).group(1).strip() + "\n\n"
new_ch2 += "\\section{Weight Distribution}\n"
new_ch2 += re.search(r"\\section\{Weight Distribution\}(.*?)\\section\{Feature Space: \\texttt\{x0\}--\\texttt\{x85\}\}", ch3_merged, re.DOTALL).group(1).strip() + "\n\n"
new_ch2 += "\\section{Data Pre-processing}\n"
new_ch2 += re.search(r"\\section\{Feature Space: \\texttt\{x0\}--\\texttt\{x85\}\}(.*?)\\section\{Validation Strategy\}", ch3_merged, re.DOTALL).group(1).strip() + "\n"
new_ch2 += "Handling missing values and normalization are intrinsically tied to the feature space. "
new_ch2 += re.search(r"\\section\{Missing Value Analysis\}(.*?)\\section\{Distributional Analysis of the Target\}", ch3_merged, re.DOTALL).group(1).strip() + "\n\n"
new_ch2 += "\\section{Exploratory Analysis}\n"
new_ch2 += re.search(r"\\section\{Distributional Analysis of the Target\}(.*?)\\section\{EDA Summary and Modeling Implications\}", ch3_merged, re.DOTALL).group(1).strip() + "\n\n"
new_ch2 += "\\section{EDA Summary}\n"
new_ch2 += re.search(r"\\section\{EDA Summary and Modeling Implications\}(.*)", ch3_merged, re.DOTALL).group(1).strip() + "\n\n"
...
new_ch3 += re.search(r"\\section\{Validation Strategy\}(.*?)\\chapter\{Exploratory Data Analysis\}", ch3_merged, re.DOTALL).group(1).strip() + "\n\n"
new_ch3 += "\\section{Evaluation Metric}\n"
new_ch3 += re.search(r"\\section\{Evaluation Metric: Weighted Skill Score\}(.*?)\\section\{Data Schema\}", ch1_merged, re.DOTALL).group(1).strip() + "\n\n"
...
new_ch3 += re.search(r"\\section\{Architecture Overview\}(.*?)\\section\{Model Comparison\}", ch6_merged, re.DOTALL).group(1).strip() + "\n\n"
...
new_ch3 += re.search(r"\\subsection\{Hyperparameters\}(.*?)\\section\{Model Comparison\}", ch6_merged, re.DOTALL).group(1).strip() + "\n\n"
new_ch3 += "\\section{Probabilistic Forecasting Models}\n"
new_ch3 += re.search(r"\\begin\{figure\}.*?slide_42\.png.*?\\end\{figure\}", ch6_merged, re.DOTALL).group(0) + "\n"
...
new_ch4 += re.search(r"\\chapter\{Results and Analysis\}(.*?)\\section\{Error Analysis\}", ch6_merged, re.DOTALL).group(1).strip() + "\n\n"
new_ch4 += "\\section{Error Analysis}\n"
new_ch4 += re.search(r"\\section\{Error Analysis\}(.*)", ch6_merged, re.DOTALL).group(1).strip() + "\n\n"
new_ch4 += "\\section{Research Gap Identified}\n"
new_ch4 += re.search(r"\\section\{Limitations of Classical Models\}(.*)", ch5, re.DOTALL).group(1).strip() + "\n"
new_ch4 += re.search(r"\\section\{Challenges and Considerations\}(.*?)\\chapter\{Results and Analysis\}", ch6_merged, re.DOTALL).group(1).strip() + "\n\n"
